...
console.log("SUPABASE_ANON_KEY:", supabaseKey ? "✓ Configurada" : "✗ Não encontrada");
...
const appRouter = router({
  bombeiros: router({
    ...
    list: publicProcedure.query(async () => {
      console.log("📋 Listando bombeiros...");
      ...
      console.log("✓ Bombeiros listados:", data?.length || 0);
      return data || [];
    }),
    ...
    create: publicProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          console.log("➕ Criando novo bombeiro:", input.nome);
          const { data, error } = await supabase
            .from("bombeiros")
            .insert([{
          ...
          console.log("✓ Bombeiro criado com sucesso");
          return data?.[0];
        } catch (e: any) {
    ...
    delete: publicProcedure
      ...
      .mutation(async ({ input }) => {
        console.log("🗑️ Deletando bombeiro:", input.id);
        ...
        console.log("✓ Bombeiro deletado com sucesso");
        return { success: true };
      }),
  }),
  ...
  escalas: router({
    ...
    list: publicProcedure.query(async () => {
      ...
      console.log("✓ Escalas listadas:", data?.length || 0);
      return data || [];
    }),
    ...
    create: publicProcedure
      ...
      .mutation(async ({ input }) => {
        console.log("➕ Criando nova escala");
        const { data, error } = await supabase
          .from("escalas")
          .insert([{
...
// Exportar como serverless function do Vercel
export default app;
