...
export default function Home() {
  ...
  if (isLoading) return (
    <div className="flex items-center justify-center h-screen bg-[#020617]">
      <div className="text-red-500 font-black animate-pulse uppercase tracking-widest">Sincronizando Sistema...</div>
    </div>
  );
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex justify-between items-center bg-[#1E293B] p-8 rounded-3xl border border-slate-800 shadow-2xl">
          <div>
            <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter">Efetivo de Prontidão</h2>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">Quartel de Andradina</p>
          </div>
          <button onClick={() => setModal(true)} className="bg-blue-600 hover:bg-blue-500 text-white font-black px-8 py-4 rounded-2xl flex items-center gap-2 transition-all text-xs uppercase shadow-lg shadow-blue-900/40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {bombeiros?.map((b: any) => {
            ...
            return (
              <div key={b.id} className="bg-[#0F172A] border border-slate-800 p-6 rounded-3xl flex justify-between items-center hover:border-slate-600 transition-all shadow-xl">
          ...
          <div className="fixed inset-0 bg-black/95 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-[#1E293B] border border-slate-700 w-full max-w-sm rounded-[2.5rem] p-8 space-y-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-4">
                ...
                <select value={equipe} onChange={e => setEquipe(e.target.value as any)} className="w-full bg-[#020617] border border-slate-700 rounded-2xl p-5 text-white font-bold outline-none focus:ring-2 focus:ring-blue-600">
